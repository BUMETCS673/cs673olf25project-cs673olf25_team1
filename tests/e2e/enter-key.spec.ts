...
// AT-17: Whitespace-only input
test('AT-17: should not send whitespace-only message', async ({ page }) => {
  await login(page);
    await page.goto(`${BASE_URL}/ai`);
  const countBefore = await selectors.list(page).locator('div:has-text("You:")').count();
  await selectors.input(page).fill('     ');
...
// AT-18: Rapid Enter presses
test('AT-18: should not duplicate message on rapid Enter presses', async ({ page }) => {
  const msg = 'Test ' + Date.now();
  await login(page);
    await page.goto(`${BASE_URL}/ai`);
  await selectors.input(page).fill(msg);
...
// AT-19: Shift+Enter creates newline (not send)
test('AT-19: pressing Shift+Enter creates newline in input', async ({ page }) => {
  await login(page);
    await page.goto(`${BASE_URL}/ai`);
  await selectors.input(page).fill('Line1');
  await page.keyboard.press('Shift+Enter');
...
// AT-20: Enter pressed when input not focused
test('AT-20: pressing Enter outside input does nothing', async ({ page }) => {
  await login(page);
    await page.goto(`${BASE_URL}/ai`);
  const countBefore = await selectors.list(page).locator('div:has-text("You:")').count();
  await page.click('body'); // unfocus input
...
// AT-21: Send, then Enter again with empty input
test('AT-21: pressing Enter again immediately does nothing', async ({ page }) => {
  await login(page);
    await page.goto(`${BASE_URL}/ai`);
  const msg = 'Test ' + Date.now();
  await selectors.input(page).fill(msg);
